...
This repository is the **simulation engine**, acts as an importable `vtvl_sim` package plus a CLI. It has no GUI. The interactive front-end built for SUSF Starworks lives in its own repo and consumes this engine as a dependency:
...
- **Full nonlinear planar dynamics** — 3-DOF rigid-body EOM over a 7-state vector `[x, z, ẋ, ż, θ, θ̇, m]`, mass depletion included via the Tsiolkovsky equation `ṁ = -T/(g·Isp)`.
- **Four controllers as one registry** — `AltitudePIDController` (1-DOF baseline), `AttitudePDController` (inner-loop demo), `CascadedController` (three nested PD loops), `LQRController` (trim-point linearisation + Bryson's-rule weighting, gain-scheduled on live mass). Selected and built by name via `CONTROLLER_REGISTRY`.
- **Mass depletion** — takes a propellant budget, a dedicated `propellant_expended` terminal event distinct from touchdown, and a built-in cross-check between the post-hoc thrust-integral propellant estimate and the ODE's actual integrated mass loss.
- **JSON-driven, Pydantic-validated scenarios** — physical params, controller/gains, multi-phase mission profiles, initial state, and output toggles all in `test_scenarios/*.json`, validated via Pydantic before being handed to the solver.
- **Multi-phase mission chaining** — `sim_run` chains phases into one continuous trajectory, carrying state (including mass) across phase boundaries.
- **Full diagnostic output** — trajectory, state, engine (thrust/gimbal, with a mass-varying hover reference), and propellant-usage plots. Optionally, a descent animation with a live HUD (altitude, speed, attitude, propellant remaining, throttle), a CSV export and a TXT short report.
- **14-test regression suite** — open-loop dynamics, LQR controllability/stability/gain-structure/landing, mass conservation and gain-scheduling under depletion, and the closed inner attitude loop against its linear design targets.
- **Stable, versioned public API** — `vtvl_sim`'s package root exposes a frozen surface consumed by the separate GUI repo via tagged releases (currently `v0.3.0`).

---

## Problem

A rigid-body lander in a vertical plane has state `(x, z, ẋ, ż, θ, θ̇)` and two controls: thrust magnitude `T` and gimbal angle `δ`. The plant is underactuated as it carries three translational/rotational DOF driven by two inputs. Horizontal translation is controlled through attitude, like in a quadrotor. The goal is a soft, accurate touchdown from an offset initial condition, with an accurate comparison of controllers on fuel, accuracy, and constraint satisfaction.
...
### Phase 1 — dynamics + simulator (complete)
...
### Wee# 2D VTVL Descent Simulation
...
### Phase 2 — cascaded PD (substantially complete)
...
### Phase 4 — mass depletion (complete), guidance stretch + write-up (open)
...
Still open — see **Planned features** below.

---

## Planned features

- **Guidance stretch goal (open decision)** — convex minimum-fuel guidance (G-FOLD) tracked by LQR, or a Monte Carlo dispersion analysis if guidance is skipped. `guidance.py` is an empty stub reserved for this; not yet started.
- **A deliberate gain re-tune under the depleting-mass model** (or a revisit of the propellant-margin assumption) — deferred so the mass-depletion mechanism could be verified correct first. See `REPORT_NOTES.md` §7.2 for what that would actually involve and why it isn't a free lunch: propellant-cheap and actuator-gentle trade against each other in a quadratic-cost regulator, and standard LQR carries no fuel term at all.
- **Inner-loop optional checks** — settling time and rise-time monotonicity, deprioritised in `tests/attitude_test_plan.md`'s original spec as noisy under adaptive-step integration. Not blocking; low effort if picked up later.
- **Final technical write-up** — either a section of the README or a Latex report.
...
```
k 2 — cascaded PD (substantially complete)
...
- **A deliberate gain re-tune under the depleting-mass model** (or a revisit of the propellant-margin assumption) — deferred so the mass-depletion mechanism could be verified correct first. See `REPORT_NOTES.md` §7.2 for what that would actually involve and why it isn't a free lunch: propellant-cheap and actuator-gentle trade against each other in a quadratic-cost regulator, and standard LQR carries no fuel term at all.
- **Inner-loop optional checks** — settling time and rise-time monotonicity, deprioritised in `tests/attitude_test_plan.md`'s original spec as noisy under adaptive-step integration. Not blocking; low effort if picked up later.
- **Final technical write-up** — this README becomes the short technical report once the above settle.
